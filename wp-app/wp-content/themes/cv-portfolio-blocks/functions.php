 ...
 */
function physiotherapy_clinic_admin_notice() {
    // Check if the notice is dismissed
    $dismissed = get_user_meta(get_current_user_id(), 'physiotherapy_clinic_dismissed_notice', true);
...
add_action('admin_notices', 'physiotherapy_clinic_admin_notice');

/**
 * AJAX handler to dismiss the notice.
 */
function physiotherapy_clinic_dismissed_notice() {
    // Set user meta to indicate the notice is dismissed
    update_user_meta(get_current_user_id(), 'physiotherapy_clinic_dismissed_notice', true);
    die();
}

// Hook for the AJAX action
add_action('wp_ajax_physiotherapy_clinic_dismissed_notice', 'physiotherapy_clinic_dismissed_notice');

/**
 * Clear dismissed notice state when switching themes.
 */
function physiotherapy_clinic_switch_theme() {
    // Clear the dismissed notice state when switching themes
    delete_user_meta(get_current_user_id(), 'physiotherapy_clinic_dismissed_notice');
}

// Hook for switching themes
add_action('after_switch_theme', 'physiotherapy_clinic_switch_theme');